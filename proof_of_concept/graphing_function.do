...
** legend
local legend legend(order(3 5 4 1) label(1 "95% CI") label(3 "Observed cases") labe(4 "Counterfactual") label(5 "Trend") col(4) symxsize(*.5))
...
** graph IPD
tw rarea `stub'_pred_lower `stub'_pred_upper moyr, `acolor' || `indicatorLine' || scatter `y' moyr, `mcolor' || line `stub'_pred_cf moyr, `cfcolor' lpattern(shortdash) || line `stub'_pred moyr, `settings' `title' `linelabels' `coefText'
